...
base_vect <- c(2, 4, # SIS, BGM -- can comp with 4 for MBG
               ...
               0.7, 0.85, 1, 0.005, # 23-27 reproduction & mutation
...
#                         "case" = c("\u03d2"), # recovery
#                         "compartments" = c(2))
sis_cb <- expand.grid("benefit" = c(0, 0.1, 0.2, 0.3, 1)*1, # for percent change in benefit: 0, 50, 75, 100
...
sim_results_cb <- mclapply(1:(N*rep_num), function(i){
  ...
  if (rep_sis_cb$transmission[i] == 3) {
    ...
    # cost update
    cb_vect[23] <- as.numeric(rep_sis_cb$cost[i])*1
