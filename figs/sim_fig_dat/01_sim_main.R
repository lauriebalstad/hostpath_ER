...
base_vect <- c(2, 4, # SIS, BGM -- can comp with 4 for MBG
               -1, -1, -1, 0.01, # 3-6 no enviro dep transmission
               -1, -1, -1, 0.01, # 7-10 always dens dep transmission
               0.05, 0.05, 0.05, # 11-13 + 17 background mort
               0.95, 0.95, 0.95, # 14-16 + 18 disease mort
               0.01, 0.01, # 17-18 mort sd
               0.05, 0.05, 0.05, 0.01, # 19-22 recovery, unused bc SIX (1)
               0.7, 0.95, 1, 0.005, # 23-27 reproduction & mutation
               # 0.6, 0.95, 1, 0.05, 0.005, # 23-27 reproduction & mutation
               120, 4, # 28-29 carrying capacity things
               160, 3, # 30-32 timing things # note change from d = 1 to d = 2???
...
#                exp(0.0866)*0.6, exp(0.0866)*0.95, exp(0.0866), 0.0005, # 23-27 reproduction & mutation
#                # 0.6, 0.95, 1, 0.05, 0.005, # 23-27 reproduction & mutation
...
nden_vect <- c(2, 4, # SIS, BGM -- can comp with 4 for MBG
               ...
               120, 4, # 28-29 carrying capacity things
               160, 3, # 30-32 timing things # note change from d = 1 to d = 2???
               0.05, 1, 160) # init R and init disease, ngens
...
#                         "case" = c("\u03d2"), # recovery
#                         "compartments" = c(2))
sis_cb <- expand.grid("benefit" = c(0, 0.1, 0.2, 0.3, 1)*0.95, # for percent change in benefit: 0, 50, 75, 100
                      "cost" = c(0.4, 0.7, 1), # 0.1 = some cost, 1 = no cost
                      # "case" = c("\u03bc"), # mortality
                      # "compartments" = c(2),
                      "transmission" = 1:3)
...
sim_results_cb <- mclapply(1:(N*rep_num), function(i){
  ...
  if (rep_sis_cb$transmission[i] %in% c(1, 2)) {
    ...
    if (rep_sis_cb$transmission[i]==1) {
      ...
      cb_vect[7:9] <- 1 # transmission increased (dens)
    }
    if (rep_sis_cb$transmission[i]==2) {
      cb_vect[3:5] <- 0.05 # both types of transmission
      cb_vect[7:9] <- 1 # transmission increased (dens)
    }
...
sim_results_str <- mclapply(1:(N*rep_num), function(i){
  ...
  if (rep_cases$transmission[i] == 3) {
    ...
    if (rep_cases$robustness[i] == 3) {
      case_vect[19] <- 0.05*2 # about doubles probability of recovery compared to regular case?
  ...
  if (rep_cases$transmission[i] %in% c(1, 2)) {
    ...
    if (rep_cases$transmission[i]==1) {
      ...
      case_vect[7:9] <- 1 # transmission increased (dens)
      if (rep_cases$robustness[i] == 2) {case_vect[7:9] <- c(0, 0.5, 1)}
    }
    if (rep_cases$transmission[i]==2) {
      case_vect[3:5] <- 0.05 # both types of transmission
      case_vect[7:9] <- 1 # transmission increased (dens)
      if (rep_cases$robustness[i] == 2) {
        case_vect[3:5] <- c(0, 0.025, 0.05)
        case_vect[7:9] <- c(0, 0.5, 1)
        }
    }
...
sim_results_pm <- mclapply(1:(N*rep_num), function(i){
  ...
  if (rep_sis_pm$transmission[i] %in% c(1, 2)) {
    ...
    if (rep_sis_pm$transmission[i]==1) {
      case_vect[3:5] <- -1 # no outside infection
      case_vect[7:9] <- 1 # transmission increased (dens)
      if (rep_sis_pm$robustness[i] == 2) {case_vect[7:9] <- c(0, 0.5, 1)}
    }
...
sis_dc <- expand.grid("disease cycles" = c(2:5), # aiming to span a bit of a range wihtout too many simulations
                      "robustness" = c(1), # M, B only
                      "event order" = c(1, 4, 6),
                      "transmission" = 1:3,
...
sim_results_dc <- mclapply(1:(N*rep_num), function(i){
  ...
  if (rep_sis_dc$transmission[i] == 3) {
    ...
    if (rep_sis_dc$robustness[i] == 2) {
      case_vect[7:9] <- c(-2, -2, Inf) # only consider density-dependent, takes care of probability issue
      case_vect[3:5] <- c(0, -log(1-0.5), Inf) # probability of infection for WR should be 0.5ish
    }
    # recovery
    if (rep_sis_dc$robustness[i] == 3) {
  ...
  if (rep_sis_dc$transmission[i] %in% c(1, 2)) {
    ...
    if (rep_sis_dc$transmission[i]==1) {
      case_vect[3:5] <- -1 # no outside infection
      case_vect[7:9] <- 1 # transmission increased (dens)
      if (rep_sis_dc$robustness[i] == 2) {case_vect[7:9] <- c(0, 0.5, 1)}
    }
    if (rep_sis_dc$transmission[i]==2) {
      case_vect[3:5] <- 0.05 # both types of transmission
      case_vect[7:9] <- 1 # transmission increased (dens)
