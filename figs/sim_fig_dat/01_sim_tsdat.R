...
coexist_parsA <- c(2, 4, # SIS, BGM -- can comp with 4 for MBG
                   0, 0.1, 0.2, 0.01, # 3-6 no enviro dep transmission
                   0, 1, 2, 0.01, # 7-10 always dens dep transmission
                   0.15, 0.15, 0.15, # 11-13 + 17 background mort
                   1.3, 1.3, 1.3, # 14-16 + 18 disease mort
                   0.01, 0.01, # 17-18 mort sd
                   0.05, 0.05, 0.05, 0.01, # 19-22 recovery, unused bc SIX (1)
                   0.6, 0.6, 0.6, 0.005, # 23-27 reproduction & mutation
                   # 0.6, 0.8, 1, 0.05, 0.005, # 23-27 reproduction & mutation
                   100, 4, # 28-29 carrying capacity things
                   120, 1, # 30-32 timing things # note change from d = 1 to d = 2???
                   0.05, 0.1, 160) # init R and init disease, ngens
coexist_parsB <- c(1, 6, # SIS, BGM -- can comp with 4 for MBG
                   0.1, 0.1, 0.1, 0.01, # 3-6 no enviro dep transmission
                   2, 2, 2, 0.01, # 7-10 always dens dep transmission
                   0.15, 0.15, 0.15, # 11-13 + 17 background mort
                   0.1, 0.1, 2, # 14-16 + 18 disease mort
                   0.01, 0.01, # 17-18 mort sd
                   0.05, 0.05, 0.05, 0.01, # 19-22 recovery, unused bc SIX (1)
                   0.5, 0.5, 0.6, 0.005, # 23-27 reproduction & mutation
                   # 0.6, 0.8, 1, 0.05, 0.005, # 23-27 reproduction & mutation
                   100, 4, # 28-29 carrying capacity things
                   120, 1, # 30-32 timing things # note change from d = 1 to d = 2???
                   0.05, 0.1, 160) # init R and init disease, ngens
back_pars <- c(2, 1, # SIS, BGM -- can comp with 4 for MBG
               -2, -2, -2, 0.01, # 3-6 no enviro dep transmission
               0, 0, 1.7, 0.01, # 7-10 always dens dep transmission
               0.15, 0.15, 0.15, # 11-13 + 17 background mort
               2.1, 2.1, 2.1, # 14-16 + 18 disease mort
               0.01, 0.01, # 17-18 mort sd
               0.05, 0.05, 0.05, 0.01, # 19-22 recovery, unused bc SIX (1)
               0.3, 0.3, 0.6, 0.005, # 23-27 reproduction & mutation
               # 0.6, 0.8, 1, 0.05, 0.005, # 23-27 reproduction & mutation
               100, 4, # 28-29 carrying capacity things
               120, 1, # 30-32 timing things # note change from d = 1 to d = 2???
               0.05, 0.1, 160) # init R and init disease, ngens
ext_pars <- c(1, 3, # SIS, BGM -- can comp with 4 for MBG
              0.1, 0.1, 0.1, 0.01, # 3-6 no enviro dep transmission
              2, 2, 2, 0.01, # 7-10 always dens dep transmission
              0.15, 0.15, 0.15, # 11-13 + 17 background mort
              1, 1.6, 2.2, # 14-16 + 18 disease mort
              0.01, 0.01, # 17-18 mort sd
              0.05, 0.05, 0.05, 0.01, # 19-22 recovery, unused bc SIX (1)
              0.1, 0.3, 0.5, 0.005, # 23-27 reproduction & mutation
              # 0.6, 0.8, 1, 0.05, 0.005, # 23-27 reproduction & mutation
              100, 4, # 28-29 carrying capacity things
              120, 1, # 30-32 timing things # note change from d = 1 to d = 2???
              0.05, 0.1, 160) # init R and init disease, ngens
demo_parsA <- c(3, 2, # SIS, BGM -- can comp with 4 for MBG
                -2, -2, -2, 0.01, # 3-6 no enviro dep transmission
                0, 1, 2, 0.01, # 7-10 always dens dep transmission
                0.15, 0.15, 0.15, # 11-13 + 17 background mort
                1.7, 1.7, 1.7, # 14-16 + 18 disease mort
                0.01, 0.01, # 17-18 mort sd
                0.15, 0.1, 0.05, 0.01, # 19-22 recovery, unused bc SIX (1)
                0.6, 0.6, 0.6, 0.005, # 23-27 reproduction & mutation
                # 0.6, 0.8, 1, 0.05, 0.005, # 23-27 reproduction & mutation
                100, 4, # 28-29 carrying capacity things
                120, 1, # 30-32 timing things # note change from d = 1 to d = 2???
                0.01, 0.05, 160) # init R and init disease, ngens
demo_parsB <- c(2, 6, # SIS, BGM -- can comp with 4 for MBG
                -2, -2, -2, 0.01, # 3-6 no enviro dep transmission
                0, 1.1, 2.2, 0.01, # 7-10 always dens dep transmission
                0.15, 0.15, 0.15, # 11-13 + 17 background mort
                1.7, 1.7, 1.7, # 14-16 + 18 disease mort
                0.01, 0.01, # 17-18 mort sd
                0.05, 0.05, 0.05, 0.01, # 19-22 recovery, unused bc SIX (1)
                0.4, 0.5, 0.6, 0.005, # 23-27 reproduction & mutation
                # 0.6, 0.8, 1, 0.05, 0.005, # 23-27 reproduction & mutation
                100, 4, # 28-29 carrying capacity things
                120, 1, # 30-32 timing things # note change from d = 1 to d = 2???
                0.01, 0.05, 160) # init R and init disease, ngens
...
no_dens_pars <- c(2, 4, # compartments, event order
                  Inf, Inf, Inf, 0.01, # 3-6 no enviro dep transmission
                  Inf, Inf, Inf, 0.01, # 7-10 always dens dep transmission
                  0.15, 0.15, 0.15, # 11-13 + 17 background mort 
                  0.5, 0.75, 1, # 14-16 + 18 disease mort
                  0.01, 0.01, # 17-18 mort sd
                  -2, -2, -2, 0.01, # 19-22 recovery, unused bc SIX (1)
                  0.9, 0.95, 1, 0.005, # 23-27 reproduction & mutation
                  # 0.6, 0.8, 1, 0.05, 0.005, # 23-27 reproduction & mutation
                  200, 4, # 28-29 carrying capacity things
                  120, 1, # 30-32 timing things # note change from d = 1 to d = 2???
                  0.05, 0.1, 160) # init R and init disease, ngens
...
# combine
ts_pars <- rbind(coexist_parsA, coexist_parsB, 
                 coexist_parsB, ext_pars, 
                 coexist_parsA, back_pars,
                 back_pars, ext_pars,
                 demo_parsA, demo_parsB, 
...
all_ts_dat$ts <- all_ts_dat$ts - 120 # 100 pre-disease intro steps
...
saveRDS(all_ts_dat, file = "dat/all_ts_dat_0605.rds")
