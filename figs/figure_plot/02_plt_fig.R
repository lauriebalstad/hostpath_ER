...
library(viridis)
library(ggh4x)
library(rattle) # NB: overwrite of importance from ranger/randomForest
...
str_dat <- readRDS("dat/str_fig_1020.Rdata")
# convert to df
...
cb_dat <- readRDS("dat/cb_fig_1020.Rdata")
...
dc_dat <- readRDS("dat/dc_fig_1021.Rdata")
...
pm_dat <- readRDS("dat/pm_fig_1020.Rdata")
...
ggparty(as.party(rp_clss), horizontal = TRUE) +
  geom_edge() +
  geom_edge_label() +
  geom_node_splitvar() +
...
fig1Bii <- ggplot(data = RF_imp, aes(`imprt`, reorder(neat_names, `imprt`))) + 
...
fig1Bi <- ggplot(data = RF_dist, aes(clss_lvl, fill = clss_lvl)) + geom_bar(col = "black") + 
...
# save figure
# png("figs/figure_plot/structural_overview_0926.png",height=145,width=170,res=400,units='mm')
# print(fig1)
# dev.off()
fig1
...
# save figure
# png("figs/figure_plot/cost_benefit_0926.png",height=175,width=170,res=400,units='mm')
# print(fig2)
# dev.off()
fig2
...
tmp[is.na(tmp)] <- 0
pm_summ_long <- pivot_longer(tmp[, c(2:7, 9)], cols= 1:2, names_to = "outcome", values_to = "value")
...
fig3 <- ggplot(NULL, aes(x=as.factor(`population size`), y=value, col=as.factor(`mutation rate`))) + 
  ...
  scale_color_manual(values = c("#ac1457","#DB6341", "#f1c4a2")) + 
  facet_grid(outcome_f ~ transmission, scales = "free_y") + 
  theme_bw() + theme(text = element_text(size = 12), legend.position = "bottom") + 
  labs(x = "Population size", col = "Mutation rate", y = NULL)
# save figure
# png("figs/figure_plot/pop_mut_0926.png",height=145,width=85,res=400,units='mm')
# print(fig3)
# dev.off()
...
# save figure
# png("figs/figure_plot/dc_supp_0925.png",height=210,width=170,res=400,units='mm')
# print(figS1)
# dev.off()
figS1
...
# points plot
figS2 <- ggplot(plot_long, aes(`evolutionary pathway`, value, col = `transmission type`)) + 
