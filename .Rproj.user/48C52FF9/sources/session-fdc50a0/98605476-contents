...
# save figure
png("figs/figure_plot/structural_overview_0203.png",height=140,width=170,res=400,units='mm')
print(fig1)
dev.off()
...
cost_ben$firstK95 <- cost_ben$firstK95-50 # remove 100 year burn in period (100/2 disease gens)
...
fig2 <- ggplot(NULL, aes(x=as.factor(benefit), y=value, col = as.factor(cost))) + 
  ...
  geom_linerange(data = cb_summ_long, aes(ymin = less, ymax = plus), lwd = 1) + 
  geom_point(data = cb_summ_long, size = 2) + 
...
# save figure
png("figs/figure_plot/cost_benefit_0203.png",height=155,width=170,res=400,units='mm')
print(fig2)
dev.off()
...
# ex_risk <- ex_risk %>% filter(compartments == 3)
# rename
...
ex_risk$firstK95 <- ex_risk$firstK95-50 # remove 50 year burn in period
...
fig3 <- ggplot(NULL, aes(x=as.factor(`population size`), y=value, col=as.factor(`mutation rate`))) + 
  ...
  facet_grid(outcome ~ compartments, scales = "free_y") + 
  theme_bw() + theme(text = element_text(size = 12), legend.position = "bottom") + 
  labs(x = "Population size", col = "Mutation rate", y = NULL)
# save figure
png("figs/figure_plot/pop_mut_full_0203.png",height=155,width=130,res=400,units='mm')
print(fig3)
dev.off()
...
dr_dat <- dc_dat %>% filter(extinct == 0 & pop_drop50 == 1 & at_K95 == 1 & r_allele_peak45 == 0) %>%    
  group_by(parm_number) %>% 
  summarize(`C - P(rescue:\ndemographic)` = n()/1000)
...
# save figure
png("figs/figure_plot/dc_0203_dens_sis.png",height=155,width=170,res=400,units='mm')
print(figS1)
dev.off()
