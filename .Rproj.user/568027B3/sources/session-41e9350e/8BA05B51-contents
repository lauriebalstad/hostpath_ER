...
parm_mat[,25] <- qunif(mat[,12], 2.1, 2.3) # WW: population should grow, on average
parm_mat[,23] <- parm_mat[,25]*qunif(mat[,18], 0.9, 1) # RR: let mat[,21] be the cost
...
parm_mat[,27] <- qunif(mat[,14], 0.005, 0.01) # mutation rate really little! 

parm_mat[,28] <- qunif(mat[,15], 70, 120) # K -- give good range
parm_mat[,29] <- qunif(mat[,16], 3, 7) # keep K sd pretty small?
parm_mat[,30] <- rep(120) # d_0 as whole number
parm_mat[,31] <- ceiling(qunif(mat[,17], 1, 5)) # 2-5 disease cycles
parm_mat[,32] <- rep(0.1) # init r small 
parm_mat[,33] <- qunif(mat[,19], 0.05, 0.15) # proportion of I individuals (set to 0.1 in rep'd sims, if at 0 means 1 individual starts as infected)
parm_mat[,34] <- rep(160) # ngens between 120-140
...
for (i in 1:dim(parm_mat)[1]) {
  ...
  if (parm_mat[i,37]==3 & parm_mat[i, 1]==1) parm_mat[i, 37] <- sample(1:2, 1)
  # don't worry if 4-7, will just become 1-2 pathways
}

# density dependence only
parm_mat[,35] <- ceiling(qunif(mat[,20], min = 0, max = 3)) # 1 = density only, 2 = freq only, 3 = both
for (i in 1:dim(parm_mat)[1]) {
...
bd_mat$benefit <- bd_mat$benefit*0.075 # note scaling
bd_mat$stand_dev <- qunif(bd_mat$stand_dev, 0.01, 0.1)
bd_mat$cost <- qunif(mat[,18], 0.9, 1)
...
saveRDS(parm_mat, "dat/mat_var_0922.Rdata")
saveRDS(bd_mat, "dat/mat_bd_0922.Rdata")
