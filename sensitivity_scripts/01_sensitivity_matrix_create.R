...
num_parms <- 25 # only need 27 unique parameters because some repeat by defintion
...
# fill in parm_mat
parm_mat[,1] <- ceiling(qunif(mat[,1], min = 0, max = 3)) # hold at SIS, vect[1] = 2
parm_mat[,2] <- ceiling(qunif(mat[,2], min = 0, max = 6)) # hold event order BGM
parm_mat[,3] <- ceiling(qunif(mat[,25], min = 0, max = 2)) # hold transmission type at density dependence
parm_mat[,4] <- 100 # "burn-in" pre-disease fixed
parm_mat[,5] <- 0 # inital proporiton of allele fixed at 0
parm_mat[,8] <- qnorm(mat[,5], 1.6, 0.05) # b_WW: transform into rate with mean 1 and sd 0.1
...
# m_IXX is additive
parm_mat[,15] <- qnorm(mat[,10], 1.1, 0.1) # m_IWW
...
parm_mat[,20] <- parm_mat[,18]*mat[,12]*rnorm(N, 0.15, 0.01) # r_WW as percent of r_RR value 
...
parm_mat[,26] <- qnorm(mat[,20], 0.008, 0.001) # mutation rate really little! 
