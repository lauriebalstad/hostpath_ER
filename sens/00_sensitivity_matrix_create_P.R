...
parm_mat[, 6] <- qunif(mat[,6], 0.001, 0.015) # compress f_sd 

parm_mat[,9] <- qunif(mat[,8], 0.8, 1.2) # b_WW: transform into rate with mean 1 and sd 0.1
parm_mat[,7] <- parm_mat[,9]*mat[,4]*0.075 # b_RR as percent of b_WW value (recall mat[7, ] is vector between 0-1)
parm_mat[,8] <- qunif(mat[,5],min=parm_mat[,7],max=parm_mat[,9]) # set b_WR between with mat[8,] dominance
parm_mat[, 10] <- qunif(mat[,6], 0.001, 0.015) # compress b_sd 

parm_mat[,11] <- qunif(mat[,9], 0.03, 0.06) # m_SXX: transform into rate with mean 0.1 and sd 0.02
parm_mat[,12] <- parm_mat[,11] # m_SWR = m_SRR
parm_mat[,13] <- parm_mat[,11] # m_SWW = m_SRR
# m_IXX is additive
parm_mat[,16] <- qunif(mat[,10], 0.8, 1.2) # m_IWW
parm_mat[,14] <- parm_mat[,16]*mat[,4]*0.075 # m_IRR as percent of m_IWW value
parm_mat[,15] <- qunif(mat[,5],min=parm_mat[,14],max=parm_mat[,16]) # set m_IWR between
parm_mat[,17] <- qunif(mat[,6], 0.001, 0.015) # compress m_Xsd 
parm_mat[,18] <- parm_mat[,17] # m_Isd = m_Ssd

parm_mat[,21] <- qunif(mat[,11], 0.03, 0.06) # r_WW: avoids negative values
parm_mat[,19] <- parm_mat[,21]*(1+(1-mat[,4]*0.075)) # r_RR: now divding by the less than one value, since RR benefits increase gamma
parm_mat[,20] <- qunif(mat[,5],min=parm_mat[,21],max=parm_mat[,19]) # set r_WR between with others with dominance
parm_mat[,22] <- qunif(mat[,6], 0.001, 0.015) # compress r_sd 

parm_mat[,25] <- qunif(mat[,12], 1, 1.2) # WW: population should grow, on average
parm_mat[,23] <- parm_mat[,25]*qunif(mat[,18], 0.5, 1) # RR: let mat[,21] be the cost
parm_mat[,24] <- qunif(mat[,5],min=parm_mat[,23],max=parm_mat[,25])
# parm_mat[,26] <- qunif(mat[,6], 0.001, 0.015) # compress l_sd 
...
parm_mat[,31] <- rep(0.05) # init r small 
parm_mat[,32] <- qunif(mat[,19], 0.025, 0.15) # proportion of I individuals (set to 0.1 in rep'd sims, if at 0 means 1 individual starts as infected)
...
# rescale as needed to match actual values
bd_mat$benefit <- (1-bd_mat$benefit*0.075) 
bd_mat$stand_dev <- qunif(bd_mat$stand_dev, 0.001, 0.015)
bd_mat$cost <- (1-qunif(mat[,18], 0.5, 1)) # rescaling
