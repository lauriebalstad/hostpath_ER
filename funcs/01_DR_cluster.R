...
dr_cluster <- function(parm_vect, init_dat, tot_ri, num_ri, ngens) { # pop size and gen time info
  ...
  t_num <- parm_vect[35] # note addt'l parameter!
  ...
  S_size <- NULL # vector of S individuals--adjust so it grabs the first element of pre_ri?? will be issue for K??
  ...
  output_dat <- c(# things to decide if ER occured
    ...
    pop_drop20 <- any(S_size+I_size+R_size < K_size*0.20), # did the population drop? T/F
    pop_drop50 <- any(S_size+I_size+R_size < K_size*0.50), # did the population drop? T/F
    pop_drop80 <- any(S_size+I_size+R_size < K_size*0.80), # did the population drop? T/F
    ...
    # get the extremes
    max_r_allele <- max(r_allele, na.rm = TRUE), # only consider post-disease hits
    time_max_r_allele <- which(r_allele==max(r_allele))[1],
    max_inf_prev <- max(I_size/(S_size+I_size+R_size), na.rm = TRUE),
    time_last_zero_inf <- which(I_size/(S_size+I_size+R_size)==0)[1], # only care about post-disease
    min_pop <- min(S_size+I_size+R_size, na.rm = TRUE), # only care about post-disease min size
    time_min_pop <- which(S_size+I_size+R_size == min_pop)[1], # time that min pop was hit
    # how long in population drop
    first_pop_drop20 <- which(S_size+I_size+R_size < K_size*0.20)[1], # first time the population dropped
    first_pop_drop50 <- which(S_size+I_size+R_size < K_size*0.50)[1], # first time the population dropped
    first_pop_drop80 <- which(S_size+I_size+R_size < K_size*0.80)[1], # first time the population dropped
    last_pop_drop20 <- which(S_size+I_size+R_size < K_size*0.20)[length(which(S_size+I_size+R_size < K_size*0.20))], # last time the population dropped
    last_pop_drop50 <- which(S_size+I_size+R_size < K_size*0.50)[length(which(S_size+I_size+R_size < K_size*0.50))], # last time the population dropped
    last_pop_drop80 <- which(S_size+I_size+R_size < K_size*0.80)[length(which(S_size+I_size+R_size < K_size*0.80))], # last time the population dropped
    total_pop_drop20 <- sum(S_size+I_size+R_size < K_size*0.20), # total time the population dropped
    total_pop_drop50 <- sum(S_size+I_size+R_size < K_size*0.50), # total time the population dropped
    total_pop_drop80 <- sum(S_size+I_size+R_size < K_size*0.80), # total time the population dropped
